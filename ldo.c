...
l_noret luaD_throw(lua_State *L, int errcode) {
    if (L->errorJmp) {  /* thread has an error handler? */
        ...
        deepret(L->errorJmp->b);  /* jump to it */
...
static void inner_prot_call(void *param) {
    struct ipc_param *iparam = (struct ipc_param *) param;
    (*iparam->f)(iparam->L, iparam->ud);
}
...
int luaD_rawrunprotected(lua_State *L, Pfunc f, void *ud) {
    ...
    L->errorJmp = &lj;
    deepcall(inner_prot_call, &(struct ipc_param) { .f = f, .L = L, .ud = ud }, &lj.b);
